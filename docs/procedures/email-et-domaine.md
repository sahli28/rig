...
| Gabarits d'e-mail hébergés | dashboard Supabase (Magic Link, Confirm sign up) | **non versionnés** — recopier à la main toute modif de `supabase/templates/*.html`, puis **vérifier par `pnpm gabarits:derive`** (`D-029`, jeton `SUPABASE_ACCESS_TOKEN` au moment du geste). Trois dérives en dix jours avaient précédé ce filet — la 3ᵉ le 12 sept., première connexion propriétaire cassée jusqu'à la recopie |
