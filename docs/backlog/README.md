...
| P1-025  | Changer les places d'UNE occurrence                |   0,75 | à faire — **priorité 1 du lot d'usage pilote (14 sept.)**. Le modèle est prêt (grants `capacity`/`is_override` admins, contrainte `booked_within_capacity` en base, refresh qui respecte le drapeau) : c'est l'écran qui n'expose pas. Champ « Places » au panneau, action dédiée, refus **avec message** sous `booked_count` |
| P1-026  | Un cours ponctuel, sans série d'un jour            |   1,25 | à faire — priorité 1. Seul ticket du lot avec migration : `schedule_id` nullable + invariant `check (schedule_id is not null or is_override)` — l'occurrence autonome est dérogatoire par construction, aucun refresh ne la balaie. Série cachée par occurrence : **écartée** (la pollution déplacée) |
| P1-027  | La liste des inscrits d'un cours, au back-office   |   1,25 | à faire — priorité 1. `class_attendance_sheet` existe (P1-008a) : à **élargir** (nom complet — décision commanditaire, `privacy.md` mis à jour dans le même commit) et à **resserrer** (COACH → ses cours, §5.2 — en base, jamais côté client). Lecture seule au web ; cocher reste au mobile |
| P1-028  | « Reprendre une séance » filtre par type de cours  |   0,25 | à faire — priorité 2. La branche « même jour » de `sourcesPourOccurrence()` propose tous les types ; une condition de plus, test avant |
| P1-029  | La séance publiée prévient ceux qui ont réservé    |    1,5 | à faire — priorité 2. **Câble un canal, n'en refait pas un** : `enqueue_push()` (quiet hours, préférences) + émetteur existent (P1-007). Valeur d'enum `WORKOUT_UPDATED` + producteur dans la même migration (une valeur d'enum sans code derrière ne vaut rien), appel depuis `saveWorkout` si le texte a changé, échec d'enfilage jamais bloquant |
| D-033   | Les boutons du panneau de séance, relus            |   0,25 | à faire — priorité 3, cosmétique, **après** P1-025/P1-027 (le panneau bouge déjà). Applique la hiérarchie D-021 : un primaire, la destruction derrière un trait |
...
|         | **Total ①**                                       | **126,5** | dont **109 faits**, **17,5 restants** |
