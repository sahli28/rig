...
| P1-024  | L'invitation en attente s'accepte sur mobile       |      1 | **bâti et prouvé au harnais le 13 sept. 2026** — **le chaînon manquant du parcours des 80**, trouvé par le test de bout en bout du 12 au soir : `accept_pending_invitation()` n'avait **aucun appelant mobile** (règle 7, le motif `create_tenant`) ; un membre importé qui se connectait dans l'app tombait sur « Aucune box pour l'instant ». L'accueil sans box liste ce qui attend l'adresse et fait rejoindre. **Cinq critères prouvés au harnais** (compte neuf → box visible → rejoint aux couleurs de Rueil ; D-016 ; expiration propre — un défaut trouvé et corrigé par ce geste). Reste le `[~]` d'appareil, derrière le build TestFlight (`P1-016`) |
| D-029   | Les gabarits d'e-mail rougissent quand ils décrochent | 0,75 | à faire — **3ᵉ dérive dashboard ↔ dépôt le 12 sept.** (première connexion propriétaire cassée jusqu'à la recopie). Le runbook dit « recopier à la main » et rien ne le vérifie : `pnpm gabarits:derive`, sœur de `heberge:derive` (P1-023), trois issues nommées. En attendant, une case de checklist (posée le 13 sept.) est la seule protection |
| D-030   | Le mapping CSV reconnaît ses propres clés          |   0,25 | à faire — constat mineur du test du 12 : `guessMapping()` reconnaît `first name` mais pas `first_name`/`last_name`, les clés **que le produit utilise lui-même**. Sans gravité (l'invitation ne lit que l'e-mail), le mapping se corrige à l'écran |
|         | **Total ①**                                       | **120,25** | dont **106 faits**, **14,25 restants** |
