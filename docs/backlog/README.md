...
| P1-015  | La séance du cours, écrite par le coach           |    5,5 | ✅ **fusionné (PR #63) — passe jouée le 9 sept. 2026, et incomplète.** Le **premier ticket venu d'un client réel**, et il tient : une séance **par occurrence**, en texte libre, avec un pré-remplissage qui **copie sans lier**, et la troisième protection contre le rafraîchissement de série. **Deux défauts à la passe, aucun visible en test** → `D-021` : le COACH est refusé par la porte du back-office, donc le droit ouvert à trois niveaux sur quatre ne s'exerce pas ; et le formulaire de la séance est logé sous le panneau d'annulation. **Le soir même, `D-021` fait et passé : A14 ✅, le critère du coach est fermé.** **Les gestes 7 et 8, rejoués le soir : NOK** — et ce n'est pas le décor (`test:db:fresh` vert, 465), c'est **le piège 13** : la policy de lecture masquait les lignes archivées à tout le monde, et PostgreSQL 17 refuse alors l'`update` qui archive. Sa sœur cassait « supprimer une série » **depuis P1-002**. Corrigé le soir même, **5 → 5,5**, à rejouer sur `main` |
