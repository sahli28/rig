...
|         | **Total ①**                                       | **120,5** | dont **108 faits**, **12,5 restants** |
...
| **`D-031` entre et sort le même jour (+0,25), 13 sept.** : le premier test TestFlight trouve un crash au lancement — build EAS sans les `EXPO_PUBLIC_*` (`.env.local` ne voyage pas). Correctif versionné + garde-fou mordant ; la re-passe TestFlight dira si l'app se lance | **120,5** | **108** | 12,5 |
