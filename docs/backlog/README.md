...
| P1-027  | La liste des inscrits d'un cours, au back-office   |      1 | à faire — priorité 1. **Décisions ratifiées le 14 sept.** (1,25 → 1) : noms complets pour **tout** le staff (`privacy.md` mis à jour dans le même commit, garde-fou kiosque posé dans `P1-008b` le même jour) ; **portée BOX conservée** pour le COACH — le strict « ses cours » (§5.2) est reporté, décision datée, à rouvrir quand une box plus grande le demandera. `class_attendance_sheet` gagne `last_name`, rien d'autre ne bouge. Lecture seule au web ; cocher reste au mobile |
...
|         | **Total ①**                                       | **126,25** | dont **109,75 faits**, **16,5 restants** |
