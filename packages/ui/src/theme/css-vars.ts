...
export function themeToCssVars(theme: Theme): Record<string, string> {
  const vars: Record<string, string> = {
    ...
    [`${PREFIX}-color-surface-3`]: theme.colors.surface3,
    ...
    [`${PREFIX}-color-scrim`]: theme.colors.scrim,
    [`${PREFIX}-color-on-image`]: theme.colors.onImage,
    [`${PREFIX}-color-on-image-muted`]: theme.colors.onImageMuted,
    [`${PREFIX}-motion-fast`]: `${theme.motion.fast}ms`,
    [`${PREFIX}-motion-base`]: `${theme.motion.base}ms`,
